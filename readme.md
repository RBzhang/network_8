...
节点内部处理逻辑运行在 **160 MHz** 主时钟域。每个光模块独占一组**异步 RX FIFO 和异步 TX FIFO**（跨时钟域），详见 [FIFO 架构](#10-fifo-架构与并行处理)。
...
- 表深度只需覆盖环上最坏往返时间内可能出现的包数（≈ 节点数 × 2），即可保证 count 回绕（65536 秒 ≈ 18h）前的旧记录已被挤出。
...
1. 光模块串行数据经 SerDes 转为 32bit 并行字，写入**异步 RX FIFO**（跨时钟域）。
...
5. 需转发或自产待发的帧，由调度器（或自有包发送逻辑）写入**异步 TX FIFO**，经 SerDes 发送到光模块。
...
- **TX 侧**持续监测异步 TX FIFO 是否非空，非空则按序取出 32bit 字发送。
- 不受 1 秒周期限制——有数据即发。
