...
module tb_node_top_4port_functional;
    ...
    reg valid_in0, valid_in1, valid_in2, valid_in3;
    ...
    node_top_4port #(
        ...
        .valid_in0(valid_in0), .valid_in1(valid_in1), .valid_in2(valid_in2), .valid_in3(valid_in3),
    ...
    task drive_word;
        ...
        begin
            ...
            case (port)
                0: begin in0 = word; valid_in0 = 1'b1; end
                1: begin in1 = word; valid_in1 = 1'b1; end
                2: begin in2 = word; valid_in2 = 1'b1; end
                3: begin in3 = word; valid_in3 = 1'b1; end
            endcase
            @(posedge clk);
        end
    endtask
    ...
    task wait_for_output_ports;
        ...
        begin
            ...
            while (((observed & required_mask) != required_mask) && cycles < TIMEOUT_CYCLES) begin
                @(posedge clk);
                cycles = cycles + 1;
                cur_v = {valid_out3, valid_out2, valid_out1, valid_out0};
                observed = observed | cur_v;
                if ((cur_v & forbidden_mask) != {NUM_PORTS{1'b0}})
                    $fatal(1, "FAIL: forbidden valid_out mask %b appeared in {%b,%b,%b,%b}",
                        forbidden_mask, valid_out3, valid_out2, valid_out1, valid_out0);
            end
    ...
    initial begin
        ...
        $display("CASE 1: in0 inject dst=1 local frame via 4port wrapper");
        inject_frame_1w(0, 8'd9, 8'd1, 16'd1, 32'hBBBB_0001);
        ...
        $display("  PASS: app_rx received local frame via 4port wrapper");

        $display("CASE 2: in0 inject dst=2 forward frame via 4port wrapper");
        inject_frame_1w(0, 8'd9, 8'd2, 16'd2, 32'hBBBB_0002);
        wait_for_output_ports(4'b1110, 4'b0001, app_rx_count_seen);
        wait_outputs_quiet(20);
        $display("  PASS: valid_out on out1/out2/out3, not out0, no app_rx leak");

        $display("CASE 3: in1 inject dst=2 forward frame via 4port wrapper");
        inject_frame_1w(1, 8'd9, 8'd2, 16'd3, 32'hBBBB_0003);
        wait_for_output_ports(4'b1101, 4'b0010, app_rx_count_seen);
        wait_outputs_quiet(20);
        $display("  PASS: valid_out on out0/out2/out3, not out1, no app_rx leak");

        $display("CASE 4: in2 inject dst=2 forward frame via 4port wrapper");
        inject_frame_1w(2, 8'd9, 8'd2, 16'd4, 32'hBBBB_0004);
        wait_for_output_ports(4'b1011, 4'b0100, app_rx_count_seen);
        wait_outputs_quiet(20);
        $display("  PASS: valid_out on out0/out1/out3, not out2, no app_rx leak");

        $display("CASE 5: in3 inject dst=2 forward frame via 4port wrapper");
        inject_frame_1w(3, 8'd9, 8'd2, 16'd5, 32'hBBBB_0005);
        wait_for_output_ports(4'b0111, 4'b1000, app_rx_count_seen);
        $display("  PASS: valid_out on out0/out1/out2, not out3, no app_rx leak");

        $display("PASS: tb_node_top_4port_functional completed");
        $finish;
    end
endmodule
